...
pub enum ItemEffect {
    FlatDamage(i32),
    ProcFlatDamage(i32, f32),
    DamageMod(f32),
    ProcDamageMod(f32, f32),
    DotDamage(i32),
    RestoreHealth(i32),
    RestoreArmor(i32),
    RestorePower(i32),
    ...
    fn test_icon_id_effect(
        ...
        for icon_id in &icon_ids {
            assert!(
                ...
                "item_mods didn't contain icon_id: item_mods = {:#?}, icon_ids = {:#?}",
                item_mods,
                icon_ids
            );
            let effects = item_mods.icon_id_effects.get_mut(&icon_id).unwrap();
            assert_eq!(test_effects.len(), effects.len(), "item_mods had a different number of effects than it should: test_effects = {:#?}, effects = {:#?}", test_effects, effects);
            ...
            for (test_effect, effect) in test_effects.iter().zip(effects.iter()) {
                assert_eq!(
                    test_effect, effect,
                    "item_mods had a different effect: test_effect = {:#?}, effect = {:#?}",
                    test_effect, effect
                );
            }
        }
        assert_eq!(
            ...
            warnings_length,
            "item_mods had an incorrect number of warnings"
        );
    }
